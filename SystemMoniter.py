...
def time_get():
  CMD = "figlet $(date|awk '{print $4}'| awk -F: '{print $1\" : \"$2}')"
  Now_time = os.popen(CMD).read()
  return Now_time
...
def Mem_get():
  CMD = "free | awk '{print $3}'| awk 'NR==2{print}'"
  CMD2 = "free | awk '{print $4}'| awk 'NR==2{print}'"
  Me_used = float(os.popen(CMD).read().replace('G\n',''))/1024/1024
  Me_free = float(os.popen(CMD2).read().replace('G\n',''))/1024/1024
...
def Cpu_use():
  CMD = "mpstat| awk '{print $4}'| tail -n 1"
  Cpu_u = os.popen(CMD).read().replace('\n','')
...
while 1 < 2:
  ...
  print(Now_power,Line_1,Now_weather,Bar_all,sep='\n')
